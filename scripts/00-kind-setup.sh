...
#
# $ NO_COLOR=1 ./scripts/00-kind-setup.sh
NO_COLOR=${NO_COLOR:-""}
if [ -z "$NO_COLOR" ]; then
  header=$'\e[1;33m'
  reset=$'\e[0m'
...
header_text "Setting up kind cluster for kaudy!"
...
header_text "Patching CoreDNS to use 8.8.8.8"
...
header_text "kind cluster is ready!"
